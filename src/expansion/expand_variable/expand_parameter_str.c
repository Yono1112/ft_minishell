...
/*   Updated: 2023/06/27 14:34:28 by yuohno           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

void	append_nbr(char **new_word, unsigned int num)
{
	if (num == 0)
...
void	expand_parameter_str(char **new_word, char **rest, char *current_word)
{
	...
	current_word += 2;
	append_nbr(new_word, g_data.last_status);
	*rest = current_word;
}
